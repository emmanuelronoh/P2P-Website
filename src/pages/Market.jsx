...
const Market = () => {
  ...
  const handleTradeClick = (trader) => {
    navigate("/amount", { state: { trader, tradeType, crypto } });
  };
  ...
  return (
    <div className="market-container">
      ...
      <div className="trader-list">
        <h2>{tradeType} {crypto.split(" - ")[0]} in {location}</h2>
        {filteredTraders.length > 0 ? (
          filteredTraders.map((trader) => (
            <div key={trader.id} className="trader-card">
              ...
              <button className={`${tradeType.toLowerCase()}-button`} onClick={() => handleTradeClick(trader)}>
